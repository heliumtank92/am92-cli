...
yargs.command(
  COMMAND,
  colorify.trace('Create Empty Backend Resource in Folders'),
  createBuilder,
  handler
)
...
async function handler(argv: Arguments) {
  ...
  const modelPath = `${rsrcPath}/${rsrcName}.Model/index.mjs`
  ...
  const controllerPath = `${rsrcPath}/${rsrcName}.Controller/index.mjs`
  ...
  const routerPath = `${rsrcPath}/${rsrcName}.Router/index.mjs`
